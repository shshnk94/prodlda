...
class VAE(object):
    ...
    def _create_network(self):
        ...
        eps = tf.random_normal(tf.shape(self.z_log_sigma_sq), 0, 1,
                               dtype=tf.float32)
    ...
    def _initialize_weights(self, n_hidden_recog_1, n_hidden_recog_2,
        ...
        all_weights['weights_gener'] = {
            'h2': tf.Variable(xavier_init(n_z, n_hidden_gener_1), name='beta')}
    ...
    def _create_loss_optimizer(self):
        ...
        self.optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate,beta1=0.99).minimize(self.cost)
    ...
    def partial_fit(self, X):
        ...
        #return cost,emb
        opt, cost = self.sess.run((self.optimizer, self.cost), feed_dict={self.x: X, self.keep_prob: .4})
        return cost 
